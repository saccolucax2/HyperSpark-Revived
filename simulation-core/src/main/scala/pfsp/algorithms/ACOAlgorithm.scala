...
abstract class ACOAlgorithm(p: PfsProblem, t0: Double, seedOption: Option[PfsSolution]) extends Algorithm {
  ...
  var T: Array[Array[Double]] = Array.ofDim[Double](p.numOfJobs, p.numOfJobs)  //desire of setting job i at jth position in solution, i and j have solution.size range
  def trail(i: Int, j: Int): Double = T(i-1)(j-1)
  seed = seedOption
  
  def initializeTrails(t0: Double): Unit = {
  ...
  def initialize(): PfsEvaluatedSolution = {
    initializeTrails(t0)
    initialSolution()
  }
  ...
  def updatePheromones(antSolution: PfsEvaluatedSolution, bestSolution: PfsEvaluatedSolution): Unit
  ...
  override def evaluate(problem: Problem, stopCond: StoppingCondition): EvaluatedSolution = {
    ...
    def loop(bestSol: PfsEvaluatedSolution, iter: Int): PfsEvaluatedSolution = {
      if(stop.isNotSatisfied) {
        ...
        var antSolution = constructAntSolution(bestSolution)
        ...
        updatePheromones(antSolution, bestSolution)
