...
class IGAlgorithm(val d:Int,val T:Double, seedOption: Option[PfsSolution]) extends Algorithm {
  ...
  private def destruction(permutation: Array[Int], d: Int): (List[Int],List[Int]) = {
    val tmp = permutation.toBuffer //used buffer because of performance, random access and changable size
  ...
  private def construction(left:List[Int], removed: List[Int], p:PfsProblem) = {
  ...
  def localSearch(permutation: Array[Int],p:PfsProblem):PfsEvaluatedSolution = {
    ...
    while(improve) {
      improve = false
      val indexes = permutation.indices.toList
      val removalOrder = random.shuffle(indexes)
      for(i <- removalOrder.indices) {
        val tmp = permutation.toBuffer
